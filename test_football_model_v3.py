...
# Set environment variable to allow loading older PyTorch models
...
def test_football_model():
    ...
    # Load the model with manual torch.load patch
    ...
    try:
        ...
        # Patch torch.load to use weights_only=False for this trusted model
        _original_torch_load = torch.load

        def patched_load(*args, **kwargs):
            # Force weights_only=False for backward compatibility with older models
    ...
    # Print comparison with generic YOLOv8n
    print("\n" + "=" * 60)
    print("📊 COMPARISON WITH GENERIC YOLOv8n")
    print("=" * 60)
    print("\nGeneric YOLOv8n (current system):")
    print("  - Classes: person, sports ball (+ 78 other COCO classes)")
    print("  - Must map: person→player, sports ball→ball")
    print("  - No goalkeeper/referee distinction")
    print("")
    print("Football-specific model (this test):")
    print(f"  - Classes: player, goalkeeper, referee, ball")
    print("  - Direct football-specific detection")
    print(f"  - Goalkeeper detection: {sum(detection_stats['goalkeeper'])} total")
    print(f"  - Referee detection: {sum(detection_stats['referee'])} total")
